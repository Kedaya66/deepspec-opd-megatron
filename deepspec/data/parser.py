...
# 渲染覆盖钩子：DeepSeek-V4 等模型不提供 jinja chat_template，
# 由使用方通过 set_render_override() 注入官方 encode_messages 渲染函数。
_RENDER_OVERRIDE = None


def set_render_override(fn):
    global _RENDER_OVERRIDE
    _RENDER_OVERRIDE = fn
...
class GeneralParser:
    ...
    def _prepare_render_messages(self, messages):
        # 启用官方渲染覆盖时，loss 前缀由官方 encoder 注入，勿在此重复添加
        if not self.assistant_loss_prefix or _RENDER_OVERRIDE is not None:
            return messages
...
def render_chat_messages(
    ...
    *,
    ...
    if _RENDER_OVERRIDE is not None:
