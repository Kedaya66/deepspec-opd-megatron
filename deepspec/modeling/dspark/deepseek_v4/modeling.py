"""DeepSeek-V4 DSpark draft models (Pro and Flash variants).

Both DeepSeek-V4 DSpark checkpoints ship the *same* architecture code and differ
only in ``config.json`` scale, so a single ``DeepSeekV4DSparkModel`` implements
the backbone and two thin subclasses (``DeepSeekV4ProDSparkModel`` /
``DeepSeekV4FlashDSparkModel``) name the variants for the trainer and evaluator
registries.

The model follows the same DSpark training/inference contract as
``Qwen3DSparkModel``: draft/noise blocks are anchored on sampled context
positions and attend bidirectionally to the projected target context and to
their own block via a flex-attention block mask; a Markov head and a confidence
(accept-rate) head refine the base logits. Only the backbone internals are
DeepSeek-V4 specific (MLA attention + MoE FFN, see ``layers.py``).
"""
...
class DeepSeekV4DSparkPreTrainedModel(PreTrainedModel):
    config_class = PretrainedConfig
    base_model_prefix = "model"
    _no_split_modules = ["DeepSeekV4DSparkDecoderLayer"]
    _supports_flex_attn = True
    _supports_sdpa = True
    # HF 默认 False;不声明的话 gradient_checkpointing_enable 直接抛 ValueError
    # (decoder 层本身是 GradientCheckpointingLayer,机制是现成的)
...
class DeepSeekV4DSparkModel(DeepSeekV4DSparkPreTrainedModel):
    def __init__(self, config) -> None:
        ...
        # Projects the concatenated target-layer hidden states into the draft
        # hidden size (the DSpark "main"/context projection).
        self.fc = nn.Linear(
    ...
    def forward(
        ...
        # MLA head_dim (e.g. 512) is too large for FlexAttention's triton kernel, so
        # DeepSeek-V4 uses the SDPA/eager path with a dense mask; keep flex support
        # for completeness when a smaller head_dim is configured.
...
class DeepSeekV4ProDSparkModel(DeepSeekV4DSparkModel):
    """DSpark draft head for the DeepSeek-V4-Pro target."""


class DeepSeekV4FlashDSparkModel(DeepSeekV4DSparkModel):
    """DSpark draft head for the DeepSeek-V4-Flash target."""
