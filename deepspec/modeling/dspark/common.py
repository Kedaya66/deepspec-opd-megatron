...
def create_dspark_attention_mask_dense(
    *,
    ...
    Equivalent to ``create_dspark_attention_mask`` but as a materialized tensor for
    the SDPA/eager path (used when the backbone has a large head_dim that
    FlexAttention's triton kernel cannot tile, e.g. DeepSeek-V4 MLA head_dim=512).
    Fully-masked query rows (dummy/dropped blocks) are pointed at kv=0 to avoid a
    NaN softmax; those positions are excluded from the loss via ``eval_mask``.
    """
    bsz, num_blocks = anchor_positions.shape
    q_len = num_blocks * block_size
    kv_len = seq_len + num_blocks * block_size
    ...
    anchor_pos = anchor_positions.gather(                              # [B, Q]
        1, q_block_id.unsqueeze(0).expand(bsz, -1)
    )
    valid = block_keep_mask.gather(                                    # [B, Q]
        1, q_block_id.unsqueeze(0).expand(bsz, -1)
    )
    is_context = kv_idx < seq_len                                     # [KV]
    is_draft = ~is_context                                            # [KV]
    kv_block_id = torch.where(
        is_draft, (kv_idx - seq_len) // block_size, torch.full_like(kv_idx, -1)
    )                                                                 # [KV]
    ...
    mask = (mask_context | mask_draft) & valid.unsqueeze(-1)          # [B, Q, KV]
    no_key = ~mask.any(dim=-1)                                       # [B, Q]
    if no_key.any():
        mask[..., 0] = mask[..., 0] | no_key
