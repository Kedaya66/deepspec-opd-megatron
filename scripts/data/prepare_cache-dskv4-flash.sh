...
model_path=/andon_AI/deepseek-ai/DeepSeek-V4-Flash 
config_path=config/dspark/dspark_dskv4-flash.py
...
train_data_path=/shenlb/zwf-spec/dataset/dataset-v1/主agent输入_医疗助手_正式环境_perfectblend_openai_conv_regen.jsonl #train_datasets/qwen3_4b/perfectblend_train_regen.jsonl
cache_dir=${HOME}/.cache/deepspec/DSK_V4_FLASH_target_cache
...
echo "Stop sglang before Step 3 if it is using the same GPUs."
echo "Step 3/3: preparing dsk-v4-flash target cache: ${cache_dir}"
CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 python scripts/data/prepare_target_cache-debug-dsk-v4-flash.py \
    --config "${config_path}" \
    --train-data-path "${train_data_path}" \
    --output-dir "${cache_dir}" \
    --local-batch-size 16
