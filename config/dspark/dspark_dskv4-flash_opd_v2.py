...
exp_name = "dspark_opd_v2_lr2e5_gbs1024"
seed = 42
...
train = dict(
    ...
    lr=2.0e-5,                       # v2:翻倍(线性缩放规则,配 gbs 1024)
    ...
    torch_compile=False,  # compile+梯度检查点在 OPD 动态形状下触发 CheckpointError(v2 首步崩),关掉走 eager 重算
)

logging = dict(
    logging_steps=1,
    # 每 global step = global_batch_size(512) 个样本;6 step ≈ 3072 样本 ≈ 每 3000 样本存一次
    checkpointing_steps=1,  # 每步存最新,方便 resume
...
online = dict(
    sglang_server_address=["127.0.0.1:30000", "127.0.0.1:30010"],  # 2xtp2 双实例
